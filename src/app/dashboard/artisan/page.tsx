...
export default function ArtisanDashboard() {
  ...
  const [imageUrl, setImageUrl] = useState(""); // AI-uploaded image
  const [title, setTitle] = useState(""); // craft title (user can provide)
  ...
  // Generate AI content (title/description/price + marketing suggestions)
  const generateAIContent = async () => {
    ...
    try {
      ...
      try {
        ...
        let marketingData: { suggestions?: string[] } | null = null;

        if (!marketingRes.ok) {
          console.error("Marketing API returned status:", marketingRes.status, marketingRes.statusText);
        } else {
          try {
            marketingData = await marketingRes.json();
          } catch (err) {
            console.error("Failed to parse Marketing API response as JSON:", err);
            marketingData = null;
          }
        }
        ...
        console.error("Error generating marketing suggestions:", err);
        setToast(err.message || "Marketing AI generation failed");
      }
    } catch (err: any) {
      console.error(err);
